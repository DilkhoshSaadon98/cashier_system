...
class OpeningEntryActionWidget extends StatelessWidget {
  final bool isMobile;
  const OpeningEntryActionWidget({super.key, required this.isMobile});

  @override
  Widget build(BuildContext context) {
    Get.put(OpeningEntryController());
    Color backColor = isMobile ? white : primaryColor;
    Color textColor = isMobile ? primaryColor : white;
    return GetBuilder<OpeningEntryController>(builder: (controller) {
      return Container(
        color: backColor,
            ...
            children: [
              ListView(
                children: [
                  ...
                  if (!isMobile) ...[
                    CustomHeaderScreen(
                      title: TextRoutes.openingEntry.tr,
                      showBackButton: true,
                      imagePath: AppImageAsset.openingEntryIcons,
                    ),
                    verticalGap(),
                  ],
                  Text(
              ...
              screenActionButtonWidget(
                ...
                backColor: backColor,
                onPressedSearch: () {
                  controller.fetchOpeningEntryData(isInitialSearch: true);
                },
                onPressedAdd: () {
                ...
                onPressedRemove: () {
                  if (controller.selectedRows.isNotEmpty) {
                    showDeleteDialog(
                        ...
                        onPressed: () async {
                          await controller
                              .removeOpeningEntryDatas(controller.selectedRows);
