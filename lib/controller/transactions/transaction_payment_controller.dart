...
class TransactionPaymentController extends TransactionController {
  clearSearchFileds() {
    ...
    getPaymentTransactionData(isInitialSearch: true);
  }
...
//! --------------------------------------------------------
  ...
  Future<void> getPaymentTransactionData({
    ...
    try {
      ...
      String? trNumber = transactionNumber.text.isNotEmpty
          ...
          : null;
      final response = await transactionClass.getTransactionsPaymentData(
        isAsc: sortAscending,
...
//? Add TransactionData:
  addPaymentTransactionDatas(BuildContext context) async {
    try {
      ...
      if (response > 0) {
        ...
        clearTransactionFields("payment");
        getPaymentTransactionData(isInitialSearch: true);
      } else {
...
//? Update TransactionData:
  ...
  Future<String> generateVoucherNumber() async {
    ...
    WHERE transaction_number LIKE 'TP-$currentYear-%'
  ...
  @override
  void onInit() {
    getSourceAccountData("payment");
    setDefaultAccountsForType('payment');
