...
class OpeningEntryController extends TransactionController {
  clearSearchFileds() {
    ...
    fetchOpeningEntryData(isInitialSearch: true);
  }
  ...
  Future<void> fetchOpeningEntryData({
    ...
    try {
      if (isInitialSearch) {
        ...
        transactionOpeningEntryData.clear();
        itemsOffset = 0;
        itemsPerPage = 50;
        showSnackBar = true;
      }
      ...
      String? trNumber = transactionNumber.text.isNotEmpty
          ? transactionNumber.text.trim()
          : null;
      final response = await transactionClass.getOpeningEntryData(
        isAsc: sortAscending,
  ...
  addOpeningEntryDatas() async {
    try {
      ...
      if (response > 0) {
        ...
        clearTransactionFields("opening_entry");
        fetchOpeningEntryData(isInitialSearch: true);
      } else {
...
//? Delete transaction Data:
  removeOpeningEntryDatas(List<int> transactionIds) async {
    try {
      int response =
...
//? Update TransactionData:
  ...
  Future<String> generateVoucherNumber() async {
    ...
    WHERE transaction_number LIKE 'OE-$currentYear-%'
  ...
  ''');
    ...
    return 'OE-$currentYear-$formattedNumber';
  }

  @override
  void onInit() {
    fetchOpeningEntryData(isInitialSearch: true);
    getSourceAccountData("opening_entry");
    super.onInit();
  }
}
